...
//выбрать так, чтобы суммарное расстояние, которое проезжают ученики от своих домов до 
//школы, было минимально.
//
//План деревни можно представить в виде прямой, в некоторых целочисленных точках которой 
//находятся дома учеников.Школу также разрешается строить только в целочисленной точке 
//этой прямой(в том числе разрешается строить школу в точке, где расположен один из домов –
...
//возрастающем порядке координаты домов учеников — целые числа, не превосходящие
//2 × 109 по модулю.
//
//Формат вывода
...
int main() {
    ...
    vector<int> v;
    int number = 0;
    //записали координаты домов учеиков
    for (int i = 0; i < N; i++) {
        cin >> number;
        v.push_back(number);
    }
    //найдем серединтй элемент вектора
    int mean = 0;
